...
class AlloyStrategy(BaseProcessStrategy):
    """
    Simplified Alloy Strategy: Transfer quantities from inputs to outputs.

    Rules:
      - Decrease input product quantities
      - Increase output product quantities
      - No complex purity calculations
    """
    ...
    def calculate(self):
        ...
        # Input mahsulotlardan kamaytirish
        self._process_inputs(process)

        # Output mahsulotlarni yaratish/yangilash
        self._process_outputs(process)
    ...
    def _process_inputs(self, process):
        """Input product'lardan quantity kamaytirish"""
        inputs = process.inputs.select_related("material", "product").select_for_update()

        for inp in inputs:
            if not inp.quantity or inp.quantity <= 0:
                raise ValidationError("Input quantity must be positive")

            # Agar product mavjud bo'lsa, quantity ni kamaytirish
            if inp.product:
                if inp.product.quantity < inp.quantity:
                    raise ValidationError(f"Not enough product: {inp.product}")

                inp.product.quantity -= inp.quantity
                inp.product.save(update_fields=["quantity"])

    def _process_outputs(self, process):
        ...
        for output in outputs:
            if not output.material:
                raise ValidationError("Output material is required")

            if not output.quantity or output.quantity <= 0:
                raise ValidationError("Output quantity must be positive")
    ...
    def _finalize_process(self, process):
...
# ========================================================================================
# OLD ALGORITHM (COMMENTED OUT FOR FUTURE REFERENCE)
# ========================================================================================
# """
# Gold purity lowering strategy with complex calculations.
# Example: 999 -> 585
#
# INPUT ROLES:
#   - BASE_GOLD (gold 999)
